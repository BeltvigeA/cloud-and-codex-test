...
class StatusReporter:
    ...
    def report_status(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if progress is not None:
                    self.log.info(
                        f"✅ Status reported for {printer_serial}: {status_str} @ {progress}%"
                    )
                else:
                    self.log.info(
                        f"✅ Status reported for {printer_serial}: {status_str}"
                    )

                return result
            else:
                self.log.warning(
                    f"Failed to report status for {printer_serial}: "
                    f"HTTP {response.status_code} - {response.text[:200]}"
                )
                return None

        except requests.RequestException as e:
            self.log.error(f"Network error reporting status for {printer_serial}: {e}")
            return None
        except Exception as e:
            self.log.error(f"Error reporting status for {printer_serial}: {e}")
            return None
    ...
    def report_offline(
        ...
        try:
            ...
            if response.status_code == 200:
                result = response.json()
                self.log.debug(f"Reported offline status for {printer_serial}")
                return result
            else:
                self.log.warning(
                    f"Failed to report offline status for {printer_serial}: "
                    f"HTTP {response.status_code}"
                )
                return None

        except requests.RequestException as e:
            self.log.debug(f"Network error reporting offline status for {printer_serial}: {e}")
            return None
        except Exception as e:
            self.log.error(f"Error reporting offline status for {printer_serial}: {e}")
            return None
