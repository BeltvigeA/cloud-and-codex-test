...
* **`legacy`** – the historical implicit FTPS uploader. Direct MQTT control is no longer supported under the API-only policy.
* **`bambuApi`** – the recommended path that delegates uploads and printer control to the official `bambulabs_api` package.

Set `lanStrategy` to `bambuApi` in the printer configuration to switch to the official client. When the dependency is not available, the code raises a clear error so operators know they must install `bambulabs_api` (for example via `pip install bambulabs_api rich`).
...
3. Install the optional dependencies where the client runs:
   ```bash
   pip install bambulabs_api rich
   ```
4. Extend the printer entry in the local configuration with:
   ...
   ```
5. Dispatching a job now leverages the official upload path, including printer control, entirely through the supported API.
